...
$qData = ($role==='admin') ?
    mysqli_query($koneksi, "SELECT o.*, u.username FROM produksi_olahan o JOIN user u ON o.id_user=u.id_user ORDER BY tanggal DESC")
    :
    mysqli_query($koneksi, "SELECT * FROM produksi_olahan WHERE id_user='$id_user_session' ORDER BY tanggal DESC");
...
<main class="flex-1 p-6">
    ...
    <a href="tambah.php" class="bg-blue-600 text-white px-4 py-2 rounded mb-4 inline-block">+ Input Produksi Olahan</a>
        ...
        <table class="w-full border border-gray-200">
            <thead class="bg-gray-100">
                <tr>
                    ...
                    <?php if($role==='admin') echo '<th class="border px-2 py-1">Peternak</th>'; ?>
                    <th class="border px-2 py-1">Jenis Olahan</th>
                    <th class="border px-2 py-1">Bahan Baku (Liter)</th>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="border px-2 py-1"><?= ucfirst($row['jenis_olahan']) ?></td>
                    <td class="border px-2 py-1"><?= number_format($row['bahan_baku_liter'],2) ?></td>
                    <td class="border px-2 py-1"><?= number_format($row['jumlah_hasil'],2) ?></td>
                    <td class="border px-2 py-1"><?= ucfirst($row['status_produksi']) ?></td>
